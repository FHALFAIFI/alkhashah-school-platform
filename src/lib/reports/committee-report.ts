...
export async function generateCommitteeReport(opts: { committeeId: string; issuedBy: string }) {
  ...
  <h2>بيانات اللجنة</h2>
  <table>
    <tr><th style="width:22%">الاسم</th><td>${esc(c.nameAr)}</td></tr>
  ...
  <table>
    ...
    ${members.map((m, i) => `<tr><td>${i + 1}</td><td>${esc(m.name)}</td><td>${esc(m.position ?? "—")}</td><td>${esc(m.role)}</td></tr>`).join("")}
  </table>
      ...
      <table>
        ...
        ${os
          .map((o) => {
            ...
            return `<tr><td>${esc(o.outcomeType)}</td><td>${esc(o.text)}</td><td>${esc(action)}</td></tr>`;
          })
          .join("")}
