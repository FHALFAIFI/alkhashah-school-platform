...
Roughly the first half of the scope is implemented, tested and green on the automated gates.
...
| 4 | Report builder | **Not started** |
| 4.5 | Saved report templates (+ migration, permissions, audit) | **Not started** |
...
| 10 | Maintenance/inspection filters and builder domains | **Not started** |
| 11 | Budget filters, reports, filter-aware summary cards | **Not started** |
...
| 16 | Permissions review | **Partly** — one real leak found and fixed (see below); builder/template permissions not designed |
| 17 | Audit review | **Partly** — deletion audit extended; template/report-generation audit not designed |
| 18 | Database design | **Partly** — migration 0031 only; template tables not designed |
| 19 | Unit / integration / E2E | **Partly** — 936 unit+integration green, 14 new; **none of the 26 mandatory browser scenarios written** |
...
Purely additive: no column dropped, renamed or retyped; no row written, deleted or rewritten.
`progress_snapshot` is retained with its historical values and is no longer written or read
(D-054). Ledger would move **31 → 32**, tables unchanged at **88**.
...
| `npm test` | **936 / 936** across 97 files (was 915 / 95 at the v2.4.1 baseline) |
...
1. Finish §4 (report builder + saved templates, with its migration, permissions and audit),
   §10, §11, §12/§13.
