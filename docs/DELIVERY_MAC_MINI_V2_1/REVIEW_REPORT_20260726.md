...
- **No default/hardcoded credentials** — seed generates a random per-account password (argon2id), written only to git-ignored `storage/private/initial-credentials.txt` (0600); temp passwords were rotated at go-live.
...
**LOW:** `الشواهد المطلوبة` on program detail is verbatim source-plan text (not a quota — optional relabel);
login lockout message enables username enumeration; login rate-limit collapses to one global bucket on
direct LAN HTTP (+ lockout DoS); MFA available but not enforced; server actions surface raw `e.message`
(English, not stack/SQL) to authenticated staff on unexpected errors; `.env` mode 644 on the dev machine.
