/**
 * v2.5.0 §20 — RTL / visual validation across the four required widths.
 *
...
const ROUTES = [
  ...
  "/people",
  // v2.5.0 §20: الأسطح الجديدة تُقاس بالمقاييس نفسها — لوحة المرشّحات ومتعدّد الاختيار
  // والشرائح والمنشئ والجداول الطويلة والأقسام المجمَّعة كلها هنا.
  "/reports",
  "/reports/builder",
  "/reports/builder?report=programs-by-domain",
  "/reports/templates",
  "/reports/individual",
  "/reports?category=committees&report=committee-registry-detailed",
  "/reports?category=meetings&report=meetings-registry-detailed",
  "/reports?category=performance&report=perf-results",
  "/reports?category=plan&report=plan-followups",
  "/reports?category=building&report=maintenance-register",
];
...
for (const w of WIDTHS) {
  ...
  await page.goto(BASE + "/plan/consistency", { waitUntil: "domcontentloaded", timeout: 120_000 });
  ...
  await page.goto(BASE + "/performance/models", { waitUntil: "domcontentloaded", timeout: 120_000 });
